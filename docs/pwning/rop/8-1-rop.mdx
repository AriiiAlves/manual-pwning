...
Syscalls são interfaces que permitem que programas em modo usuário solicitem serviços do kernel do sistema operacional, como acesso a hardware, criação de processos, gerenciamento de arquivos, etc. Para fazer uma syscall damos o número da syscall (operação que queremos), os argumentos e em seguida usamos o comando da syscall, como vemos abaixo.
...
Para ver cada syscall, seu código, parâmetros e o que faz, recomendo olhar a [Tabela de Syscalls no Linux - IME USP](https://www.ime.usp.br/~kon/MAC211/syscalls.html). Caso queira utilizar alguma, pesquise individualmente.

Os gadgets para syscall são:
