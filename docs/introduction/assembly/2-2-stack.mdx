...
void imprimir_soma(int a, int b, int c){
        printf("%d, %d, %d", a,b,c);
}
...
```assembly
[...]
; Primeiro, estabelece argumentos da nova função
mov edx, 0x3 ; Argumento 3 no registrador edx
mov esi, 0x2 ; Argumento 2 no registrador esi
mov edi, 0x1 ; Argumento 1 no registrador edi

; Então o call
call 0x400466 <imprimir_soma>

; Indo para função...
imprimir_soma:
    ; SALVAR FRAME ANTIGO
    push rbp ; Após imprimir_soma, queremos continuar a função antiga main e resgatar seu stack frame

    ; CONFIGURAR NOVO FRAME
    mov rbp, rsp ; RBP = RSP ("Saltamos" o stack frame atual, e construímos um novo stack frame agora, com uma nova base)

    ; ALOCAR ESPAÇO PARA VARIÁVEIS LOCAIS
    sub rsp, 0x10 ; Reserva espaço na pilha

    ; GUARDANDO VARIÁVEIS LOCAIS (argumentos recebido)
    mov DWORD PTR [rbp-0x4],edi
    mov DWORD PTR [rbp-0x8],esi
    mov DWORD PTR [rbp-0xc],edx

    ; NOVOS ARGUMENTOS DE FUNÇÃO
...
; AO VOLTAR 
mov DWORD PTR [rbp-0x4],0x0 ; int a = 0
mov eax, 0x0 ; return 0
leave
ret
```

Obs: Em 64-bit, os argumentos da função são passados diretamente aos registradores, em uma ordem específica para o 1°, 2° argumentos, etc. A partir o 7° argumento, começam-se a colocar os argumentos na stack, com `push <valor>`. Em 32-bit, TODOS os argumentos são inseridos na stack.
