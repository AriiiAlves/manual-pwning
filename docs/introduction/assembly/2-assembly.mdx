...
É uma redução de: `sub rsp, 8`, `mov [rsp], rax`
...
É uma redução de: `mov rax, [rsp]`, `add rsp, 8`
...
Depois que a função finaliza, **A instrução `leave` é chamada**. Sua função é **desfazer o stack frame**, e consiste em uma redução de comando:

1. `mov rbp, rsp` - Restaura RSP para a base do frame atual
2. `pop rbp` - Restaura RBP do frame anterior

Após o `leave`, **a instrução `ret` é chamada**. Ela consiste em uma redução de comando:

`pop RIP` - Desempilha topo da stack (que agora deve ser o return address) e coloca no RIP (registrador que guarda o endereço de onde está sendo executado código)

O `leave` e `ret` fazem o processo de encerramento da função.
